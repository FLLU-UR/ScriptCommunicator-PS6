...
public class InterfaceTester {
	...
	private static class TestPose implements Pose{
		...
		TestPose(Position pos, Rotation rot){
			x = pos.getX(Unit.M);
			y = pos.getY(Unit.M);
			z = pos.getZ(Unit.M);
			rx = rot.getRX(Angle.Unit.RAD);
			ry = rot.getRY(Angle.Unit.RAD);
			rz = rot.getRZ(Angle.Unit.RAD);
			lengthUnit = Unit.M;
			angleUnit = Angle.Unit.RAD;	
		}
	...
	private static class TestPoseFactory implements PoseFactory{
		...
		public Pose createPose(Position position, Rotation rotation) {
			return new TestPose(position, rotation);
		}
